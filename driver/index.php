...
// ── Xử lý check-in/out (phải xử lý TRƯỚC khi query) ─────────
...
$totalHours = (float)dCol($pdo,
    "SELECT COALESCE(SUM(work_hours),0) FROM hr_attendance
     WHERE user_id=?
       AND EXTRACT(MONTH FROM work_date)=? AND EXTRACT(YEAR FROM work_date)=?",
    [$user['id'], $thisMonth, $thisYear]);
...
.bottom-nav a {
    ...
    color: #9ca3af; font-size: .68rem; transition: color .2s; gap: 3px;
}
...
.quick-btn {
    ...
    border: none; text-align: center; color: inherit;
}
...
<div class="driver-header">
    ...
    <div class="d-flex justify-content-between align-items-end mt-1">
        <div>
            <div style="font-size:.8rem;opacity:.75;">HR Portal 👷</div>
            <div style="font-size:1.15rem;font-weight:700;"><?= htmlspecialchars($user['full_name']) ?></div>
        </div>
...
<div class="flash-wrap">
    <div class="alert alert-<?= $_SESSION['flash']['type'] ?> alert-dismissible fade show py-2 mb-0" role="alert">
        <?= $_SESSION['flash']['msg'] ?>
        ...
        <div class="d-flex gap-4 mb-3" style="font-size:.8rem;">
            ...
            <div class="text-center">
                ...
                <div class="fw-bold text-primary" style="font-size:1.1rem;">
                    <?= number_format((float)$todayLog['work_hours'], 1) ?>h
                </div>
            </div>
            <?php endif; ?>
        </div>
        ...
        <form method="POST">
            ...
            <?php elseif ($canOut): ?>
                <div class="text-muted small mb-3">
                    Đã vào lúc <strong class="text-success"><?= date('H:i', strtotime($todayLog['check_in'])) ?></strong>
                    · Đang làm việc...
                </div>
