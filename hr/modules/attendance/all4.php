...
if ($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['ajax_action']??'')==='save_att') {
    ...
    // Xác định is_late dựa trên ca (nếu có)
    $isLate = 0; $lateMin = 0;
    try {
        ...
        $shiftRow->execute([$uid,$workDate,$workDate]);
    ...
    try {
        if ($coTs) {
            ...
            $pdo->prepare("INSERT INTO hr_attendance
                ...
                VALUES (?,?,?,'present','manual',?,?,?)
                ON CONFLICT (user_id,work_date) DO UPDATE
                SET check_in=EXCLUDED.check_in, check_out=NULL,
                    work_hours=0, source='manual',
                    is_late=EXCLUDED.is_late, late_minutes=EXCLUDED.late_minutes,
                    note=EXCLUDED.note")
        ...
        echo json_encode([
            'ok'    => true,
            'msg'   => "✅ Đã lưu chấm công ngày ".date('d/m',strtotime($workDate)).$lateMsg,
            'ci'    => $checkIn,
            'co'    => $checkOut,
            'wh'    => number_format($wh,1),
            'isLate'=> $isLate,
            'latMin'=> $lateMin,
            'date'  => $workDate,
            'uid'   => $uid,
...
if ($_SERVER['REQUEST_METHOD']==='POST'
    ...
    if ($file['error']!==UPLOAD_ERR_OK){
        ...
        foreach($rows as $ln=>$row){
            ...
            try {
                if($ciTs&&$coTs){
                    $pdo->prepare("INSERT INTO hr_attendance(user_id,work_date,check_in,check_out,work_hours,status,source) VALUES(?,?,?,?,?,'present','import') ON CONFLICT(user_id,work_date) DO UPDATE SET check_in=EXCLUDED.check_in,check_out=EXCLUDED.check_out,work_hours=EXCLUDED.work_hours,source='import'")
                        ->execute([$userId,$importDate,$ciTs,$coTs,$wh]);
                } elseif($ciTs){
                    $pdo->prepare("INSERT INTO hr_attendance(user_id,work_date,check_in,status,source) VALUES(?,?,?,'present','import') ON CONFLICT(user_id,work_date) DO UPDATE SET check_in=EXCLUDED.check_in,source='import'")
...
if (isset($_GET['export'])) {
    ...
    $empExp=hrQuery($pdo,"SELECT u.id,u.full_name,u.employee_code,COALESCE(d.name,'') AS dept_name FROM users u JOIN roles r ON u.role_id=r.id LEFT JOIN departments d ON u.department_id=d.id WHERE u.is_active=TRUE AND r.name NOT IN('customer') ORDER BY d.name,u.full_name");
...
$kpiTotal=$kpiChecked=$kpiLate=$kpiNoOut=0;
...
<div class="d-flex flex-wrap gap-2 mb-2 align-items-center" style="font-size:11px;">
    ...
    <span class="ms-3 text-muted" style="font-size:10px;"><i class="fas fa-hand-pointer me-1"></i>Click vào ô để chấm công / chỉnh sửa</span>
</div>
...
<table class="table table-bordered table-sm mb-0 att-table" id="attTable">
    ...
    <tr class="table-dark">
        ...
        ?>
        <th class="text-center px-0 <?=$isSun?'col-sun':($isSat?'col-sat':'')?>  <?=$isToday?'col-today':''?>"
        ...
        <th class="text-center" style="min-width:42px;font-size:10px;">Trễ(p)</th>
    ...
    ?>
    <tr class="emp-row" data-uid="<?=$emp['id']?>" data-name="<?=htmlspecialchars($emp['full_name'])?>">
        <td class="sticky-col">
        ...
        <?php for($d=1;$d<=$daysInMon;$d++):
            ...
            // Không cho edit ngày tương lai
            $editable = !$future;
            $editAttr = $editable ? 'data-editable="1"' : '';
            ...
            if ($isSun){
                if($att&&$att['check_in']){
                    ...
                    if($att['check_out']) $cellContent.='<div class="co">'.date('H:i',strtotime($att['check_out'])).'</div>';
                ...
                $cellClass.=' cell-future'; $cellContent='';
            } elseif($leave&&!$att){
                ...
                $isLate=$att['is_late']??0; $noOut=empty($att['check_out']);
                $ci=date('H:i',strtotime($att['check_in']));
            ...
            // Data attributes cho edit
            $dataAttrs = $editable
                ? sprintf('data-date="%s" data-uid="%d" data-ci="%s" data-co="%s" data-note="%s" data-has="%d"',
        ...
        <td class="text-center <?=$st['late_minutes']>0?'text-warning':'text-muted'?> stat-cell" style="font-size:11px;"><?=$st['late_minutes']>0?$st['late_minutes'].'p':'-'?></td>
...
<table class="table table-hover table-bordered align-middle mb-0" style="font-size:.85rem">
    ...
    <tr>
        ...
        <td class="text-center" style="color:<?=$st['sunday_work']>0?'#7c3aed':'#9ca3af'?>;font-weight:<?=$st['sunday_work']>0?700:400?>"><?=$st['sunday_work']>0?$st['sunday_work']:'-'?></td>
...
<div class="modal-content">
    ...
    <div class="modal-body p-3">
        ...
        <div class="mb-3">
            ...
            <input type="time" id="attCi" class="form-control form-control-sm" placeholder="08:00">
        </div>
        <div class="mb-3">
            <label class="form-label small fw-semibold mb-1">⏰ Giờ ra</label>
            <input type="time" id="attCo" class="form-control form-control-sm" placeholder="17:00">
            <div class="form-text" style="font-size:10px;">Trống nếu chưa ra ca</div>
        </div>

        <!-- Preview giờ làm -->
        <div id="attPreview" class="rounded p-2 mb-3 d-none" style="background:#f0f9ff;border-left:3px solid #0ea5e9;font-size:11px;">
        ...
        <div class="mb-3">
            ...
            <input type="text" id="attNote" class="form-control form-control-sm" placeholder="Ghi chú (tuỳ chọn)">
        </div>
        ...
        <div class="d-flex gap-2">
            ...
            <button type="button" class="btn btn-outline-danger btn-sm" id="attDeleteBtn" title="Xóa bản ghi này">
...
/* Editable hover */
.cell-editable { cursor:pointer;transition:all .15s; }
.cell-editable:hover { filter:brightness(.92);outline:2px solid #3b82f6;outline-offset:-2px;z-index:1;position:relative; }
...
.emp-row:hover td.att-cell { filter:brightness(.97); }
...
.leg.sunday  { background:#f9f9f9;color:#9ca3af; }

/* Saving spinner overlay on cell */
.cell-saving { pointer-events:none;opacity:.5; }
...
// ── Mở modal khi click vào ô editable ───────────────────────
document.getElementById('attTable')?.addEventListener('click', function(e) {
    ...
    const df  = d.getDate().toString().padStart(2,'0') + '/' + (d.getMonth()+1).toString().padStart(2,'0') + '/' + d.getFullYear();

    document.getElementById('attModalTitle').textContent = hasRec ? '✏️ Chỉnh sửa chấm công' : '⏰ Chấm công mới';
    document.getElementById('attModalSub').textContent   = empName + ' · ' + dow + ' ' + df;
...
document.getElementById('attSaveBtn')?.addEventListener('click', async function() {
    ...
    if (!ci) {
        showAlert('warning', '⚠️ Vui lòng nhập giờ vào.');
        return;
    }
    ...
    try {
        ...
        if (data.ok) {
            ...
            // Cập nhật ô trực tiếp
            updateCell(currentCell, data);
            // Đóng modal sau 900ms
            setTimeout(()=>{
...
function updateCell(td, data) {
    if (!td) return;
    const ci = data.ci; const co = data.co;
    ...
    td.dataset.co  = co;
    td.dataset.has = '1';
    td.title = `Vào:${ci}` + (co?` Ra:${co}`:'') + (isLate?` ⚡Trễ${latMin}p`:'') + (!co?' ⚠️Chưa ra':'');
}
