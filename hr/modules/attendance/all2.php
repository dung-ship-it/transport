...
// ── Xử lý import Excel ───────────────────────────────────────
$importResult = ['success' => 0, 'error' => 0, 'messages' => []];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST'
    ...
    if ($file['error'] !== UPLOAD_ERR_OK) {
        ...
        $_SESSION['flash'] = ['type'=>'danger','msg'=>'❌ Chỉ hỗ trợ file CSV hoặc Excel (.xlsx, .xls).'];
    } else {
        // Đọc file CSV / chuyển xlsx sang csv tạm
        $rows = [];
        ...
        if ($ext === 'csv') {
            ...
            // Đọc xlsx/xls bằng ZipArchive (không cần thư viện ngoài với xlsx)
            if ($ext === 'xlsx') {
                require_once '../../../vendor/autoload.php'; // PhpSpreadsheet nếu có
                try {
                    $spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load($file['tmp_name']);
                    $sheet = $spreadsheet->getActiveSheet();
                    $firstRow = true;
                    foreach ($sheet->getRowIterator() as $row) {
                        if ($firstRow) { $firstRow = false; continue; } // bỏ header
                        $cells = [];
                        foreach ($row->getCellIterator() as $cell) {
                            $cells[] = $cell->getFormattedValue();
                        }
                        if (!empty($cells[0])) $rows[] = $cells;
                    }
                } catch (Exception $e) {
                    $_SESSION['flash'] = ['type'=>'danger','msg'=>'❌ Không đọc được file Excel. Hãy dùng CSV.'];
                    header('Location: all.php?' . http_build_query($_GET));
                    exit;
                }
            }
        }
        ...
        foreach ($rows as $lineNum => $row) {
            ...
            if (empty($empRows)) {
                $importResult['error']++;
                $importResult['messages'][] = "Dòng " . ($lineNum+2) . ": Không tìm thấy mã NV <strong>$empCode</strong>";
                continue;
            }

            $userId = $empRows[0]['id'];

            // Parse giờ — hỗ trợ HH:MM và HH:MM:SS
            $ciTs = null; $coTs = null;
            if (!empty($checkIn)) {
                $ciTs = date('Y-m-d H:i:s', strtotime("$importDate $checkIn"));
            }
            if (!empty($checkOut)) {
                $coTs = date('Y-m-d H:i:s', strtotime("$importDate $checkOut"));
            }
            ...
            if ($ciTs && $coTs) {
                ...
                $workHours = $diff > 0 ? min(round($diff / 3600, 2), 24) : 0;
            }
            ...
            try {
                if ($ciTs && $coTs) {
                    $pdo->prepare("
                        INSERT INTO hr_attendance
                            (user_id, work_date, check_in, check_out, work_hours, status, source)
                        ...
                        ON CONFLICT (user_id, work_date) DO UPDATE
                            SET check_in   = EXCLUDED.check_in,
                                ...
                                source     = 'import'
                    ")->execute([$userId, $importDate, $ciTs, $coTs, $workHours]);
                } elseif ($ciTs) {
                    $pdo->prepare("
                        INSERT INTO hr_attendance
                            (user_id, work_date, check_in, status, source)
                        VALUES (?, ?, ?, 'present', 'import')
                        ON CONFLICT (user_id, work_date) DO UPDATE
                            SET check_in = EXCLUDED.check_in,
                                source   = 'import'
                ...
                $importResult['error']++;
                $importResult['messages'][] = "Dòng " . ($lineNum+2) . " ($empCode): " . $e->getMessage();
            }
        }
        ...
        if ($importResult['success'] > 0) {
            $_SESSION['flash'] = ['type'=>'success',
                'msg'=>"✅ Import thành công <strong>{$importResult['success']}</strong> bản ghi."
                . ($importResult['error'] > 0 ? " <strong>{$importResult['error']}</strong> lỗi." : '')];
        } else {
            $_SESSION['flash'] = ['type'=>'warning', 'msg'=>'⚠️ Không import được dòng nào.'];
        }
    }

    header('Location: all.php?' . http_build_query(array_filter([
        'month'=>$viewMonth,'year'=>$viewYear,'view'=>$viewMode
    ])));
    exit;
}
...
function calcStats($uid, $attMap, $leaveMap, $otMap, $viewMonth, $viewYear, $daysInMon) {
    $st = ['work_days'=>0,'absent_days'=>0,'leave_days'=>0,'late_count'=>0,
           'late_minutes'=>0,'total_hours'=>0,'ot_hours'=>0,'sunday_work'=>0];
    for ($d=1; $d<=$daysInMon; $d++) {
        ...
        if ($dow === 7) {
            // Chủ nhật: chỉ đếm nếu có chấm công (làm thêm CN)
            if ($att && $att['check_in']) {
                $st['work_days']++;
                $st['sunday_work']++;
                $st['total_hours'] += ($att['work_hours'] ?? 0);
            }
            continue; // Không đếm absent cho CN
        }
        ...
        elseif ($att && $att['check_in']) {
            ...
            if ($att['is_late']??0) {
                $st['late_count']++;
                $st['late_minutes'] += ($att['late_minutes'] ?? 0);
            }
        } else {
            $st['absent_days']++;
        }
...
<table class="table table-bordered table-sm mb-0" style="font-size:11px;">
    ...
    <tr class="table-dark">
        ...
        <?php for($d=1;$d<=$daysInMon;$d++):
            ...
            $isSun   = $dow===7;
        ?>
        <th class="text-center px-0 <?=$isSun?'text-danger bg-dark':''?> <?=$isToday?'bg-primary text-white':''?>"
        ...
        <th class="text-center" style="min-width:38px;">CN</th>
    ...
    <tr>
        ...
        <?php for($d=1;$d<=$daysInMon;$d++):
            ...
            if ($isSun) {
                if ($att && $att['check_in']) {
                    // CN có chấm công → hiện ✓ màu tím
                    $bg = '#f3e8ff';
                    $content = '<span style="color:#7c3aed;font-weight:bold;">✓</span>';
                    if ($ot) $content .= '<br><span style="color:#6f42c1;font-size:9px;">OT</span>';
                } else {
                    $bg = '#f5f5f5';
                    $content = '<span style="color:#ccc;font-size:9px;">CN</span>';
                }
            } elseif ($future) {
                $bg = '#fafafa';
            } elseif ($leave && !$att) {
                $bg = '#e8f4fd';
                $content = '<span style="color:#0284c7;font-weight:600;">Phép</span>';
            } elseif ($att && $att['check_in']) {
                $bg      = ($att['is_late']??0) ? '#fffbf0' : '#f0fff4';
                $content = '<span style="color:'.( ($att['is_late']??0)?'#d97706':'#16a34a').';font-weight:bold;">'
                         . (($att['is_late']??0)?'⚡':'✓').'</span>';
                if ($ot) $content .= '<br><span style="color:#6f42c1;font-size:9px;">OT</span>';
            } else {
                $bg      = '#fff5f5';
                $content = '<span style="color:#dc2626;font-weight:bold;">✗</span>';
            }
        ?>
        ...
        <td class="text-center" style="color:#7c3aed;font-weight:<?=$st['sunday_work']>0?'bold':'normal'?>">
...
<form method="POST" enctype="multipart/form-data">
    ...
    <div class="modal-header border-0">
        <h6 class="modal-title fw-bold">
            <i class="fas fa-file-excel text-success me-2"></i>Import chấm công từ file
        </h6>
    ...
    <div class="modal-body">
        ...
        <div class="alert alert-info py-2 small mb-3">
            ...
            File CSV/Excel cần có 3 cột theo thứ tự:
            <strong>mã nhân viên | giờ vào | giờ ra</strong>
            (dòng đầu là header, bỏ qua tự động)
            <br>
            <a href="download_template.php" class="fw-semibold">
        ...
        <div class="mb-3">
            ...
            <div class="form-text">Tất cả bản ghi trong file sẽ được gán cho ngày này.</div>
        </div>
        ...
        <div class="mb-3">
            ...
            <div class="form-text">Hỗ trợ: .csv, .xlsx, .xls (tối đa 2MB)</div>
        </div>

        <div class="alert alert-warning py-2 small mb-0">
            <i class="fas fa-exclamation-triangle me-1"></i>
            Nếu nhân viên đã có dữ liệu ngày đó, hệ thống sẽ <strong>cập nhật đè</strong>.
        </div>
    </div>
