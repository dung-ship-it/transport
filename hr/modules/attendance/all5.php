...
if ($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['ajax_action']??'')==='save_att') {
    ...
    try {
        ...
        echo json_encode([
            'ok'=>true,'msg'=>"✅ Đã lưu chấm công ".date('d/m',strtotime($workDate)).$lateMsg,
...
<div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
    ...
    <div class="d-flex gap-2 flex-wrap">
        ...
        <a href="summary.php?month=<?=$viewMonth?>&year=<?=$viewYear?>" class="btn btn-sm btn-outline-info">
...
<div class="d-flex flex-wrap gap-2 mb-2 align-items-center" style="font-size:11px;">
    ...
    <span class="ms-3 text-muted" style="font-size:10px;">
        ...
        <?php for($d=1;$d<=$daysInMon;$d++):
            ...
            $dataAttrs=$editable
                ? sprintf('data-editable="1" data-date="%s" data-uid="%d" data-ci="%s" data-co="%s" data-note="%s" data-has="%d"',
                    $ds,$emp['id'],$ciVal,$coVal,htmlspecialchars($att['note']??'',ENT_QUOTES),$att?1:0)
                : '';
        ?>
...
<div class="modal-content">
    ...
    <div class="modal-body p-3">
        ...
        <div class="d-flex gap-2">
            ...
            <button type="button" class="btn btn-outline-danger btn-sm" id="attDeleteBtn" title="Xóa bản ghi">
...
.att-cell{height:44px;vertical-align:middle;text-align:center;padding:2px 1px;font-size:10px;line-height:1.2;}
...
.cell-editable:hover{filter:brightness(.9);outline:2px solid #3b82f6;outline-offset:-2px;z-index:1;position:relative;}
...
document.getElementById('attTable')?.addEventListener('click',function(e){
    ...
    if(!td) return;
    currentCell=td;
    const uid=td.dataset.uid,date=td.dataset.date,ci=td.dataset.ci||'',co=td.dataset.co||'',note=td.dataset.note||'';
...
function updatePreview(){
    const ci=document.getElementById('attCi').value,co=document.getElementById('attCo').value;
    const prev=document.getElementById('attPreview'),prevTxt=document.getElementById('attPreviewText');
    if(ci&&co){
        let[ch,cm]=ci.split(':').map(Number),[oh,om]=co.split(':').map(Number);
        let sm=ch*60+cm,em=oh*60+om;
        if(em<=sm) em+=1440;
        prevTxt.textContent='Số giờ làm: '+((em-sm)/60).toFixed(1)+'h';
...
document.getElementById('attSaveBtn')?.addEventListener('click',async function(){
    const uid=this.dataset.uid,date=this.dataset.date;
    const ci=document.getElementById('attCi').value,co=document.getElementById('attCo').value,note=document.getElementById('attNote').value;
    if(!ci){showAlert('warning','⚠️ Vui lòng nhập giờ vào.');return;}
    setLoading(true);
    try{
        const fd=new FormData();
        fd.append('ajax_action','save_att');fd.append('csrf_token',CSRF);
        fd.append('user_id',uid);fd.append('work_date',date);
        fd.append('check_in',ci);fd.append('check_out',co);fd.append('note',note);
        const res=await fetch('all.php',{method:'POST',body:fd});
        const data=await res.json();
        if(data.ok){
            showAlert('success',data.msg);
            updateCell(currentCell,data);
            setTimeout(()=>bootstrap.Modal.getInstance(document.getElementById('attModal'))?.hide(),900);
...
document.getElementById('attDeleteBtn')?.addEventListener('click',async function(){
    ...
    try{
        ...
        fd.append('ajax_action','save_att');fd.append('csrf_token',CSRF);
        fd.append('user_id',uid);fd.append('work_date',date);
        fd.append('check_in','');fd.append('delete_record','1');
        const res=await fetch('all.php',{method:'POST',body:fd});
        const data=await res.json();
        if(data.ok){
            if(currentCell){
                ...
                currentCell.dataset.ci='';currentCell.dataset.co='';
                currentCell.dataset.has='0';currentCell.title='Vắng';
            }
...
function updateCell(td,data){
    if(!td) return;
    const ci=data.ci,co=data.co,isLate=data.isLate==1,latMin=data.latMin;
    ...
    td.innerHTML=html;
    td.dataset.ci=ci;td.dataset.co=co;td.dataset.has='1';
...
function setLoading(on){
    ...
    if(on){btn.disabled=true;btn.innerHTML='<span class="spinner-border spinner-border-sm me-1"></span>Đang lưu...';}
    else{btn.disabled=false;btn.innerHTML='<i class="fas fa-save me-1"></i>Lưu';}
}
</script>
