...
// Nếu vừa chốt kỳ → mặc định tab revenue
...
if ($tab === 'overview') {
    ...
    // Doanh thu từ các kỳ đã chốt nằm TRONG phạm vi lọc
    ...
    // Doanh thu thẳng từ trips (fallback)
    ...
    // Chi phí nhiên liệu: cột 'amount' trong fuel_logs
    $overview['fuel_cost'] = (float)safeCol($pdo, "
        SELECT COALESCE(SUM(amount), 0) FROM fuel_logs
        WHERE log_date BETWEEN ? AND ?
    ", [$dateFrom, $dateTo]);

    // Chi phí bảo dưỡng: cột 'cost' trong vehicle_maintenance
    ...
    // Doanh thu 12 tháng gần nhất — ghép locked + trips
...
// ════════════════════════════════════════════════════════════
// 2. DOANH THU — từ statement_items (đã chốt) + trips chưa chốt
// ════════════════════════════════════════════════════════════
$revenueData    = [];
$revenueTrips   = [];
$revenueTotal   = 0;
...
if ($tab === 'revenue') {
    ...
    // Trips CHƯA thuộc kỳ đã chốt nào
    $sqlUT = "
...
if ($tab === 'cost') {
    ...
    $sqlMaint = "
        SELECT vm.maintenance_date, vm.maintenance_type, vm.description,
               vm.cost, vm.mileage, vm.garage_name,
               vm.next_maintenance_date, vm.notes,
               v.plate_number
    ...
    $costData['maint_by_type'] = safeQuery($pdo, "
        SELECT maintenance_type, COUNT(*) AS cnt, COALESCE(SUM(cost),0) AS total
...
if ($tab === 'pl') {
    ...
    foreach ($months as $m) {
        ...
        $fuelCost  = (float)safeCol($pdo, "SELECT COALESCE(SUM(amount),0) FROM fuel_logs WHERE log_date BETWEEN ? AND ?", [$ymStart, $ymEnd]);
        $maintCost = (float)safeCol($pdo, "SELECT COALESCE(SUM(cost),0) FROM vehicle_maintenance WHERE maintenance_date BETWEEN ? AND ?", [$ymStart, $ymEnd]);
...
if ($tab === 'customer') {
    ...
    foreach ($customerReport as &$row) {
        // Ưu tiên doanh thu từ kỳ đã chốt
        $lockedRev = (float)safeCol($pdo, "
            SELECT COALESCE(SUM(si.total_amount),0)
            FROM statement_items si
            JOIN statement_periods sp ON si.period_id = sp.id
...
if ($tab === 'vehicle') {
    $vehicleReport = safeQuery($pdo, "
        SELECT v.id, v.plate_number, v.brand, v.model, v.year,
               ...
               COALESCE((SELECT SUM(vm.cost) FROM vehicle_maintenance vm
                         WHERE vm.vehicle_id = v.id AND vm.maintenance_date BETWEEN ? AND ?),0) AS maint_cost,
               COALESCE((SELECT COUNT(*) FROM vehicle_maintenance vm
                         WHERE vm.vehicle_id = v.id AND vm.maintenance_date BETWEEN ? AND ?),0) AS maint_count
        FROM vehicles v
...
<!-- Biểu đồ xu hướng doanh thu 12 tháng -->
<?php if (!empty($overview['monthly'])): ?>
<div class="card border-0 shadow-sm mb-4">
    <div class="card-header bg-white py-2">
        <h6 class="fw-bold mb-0">📈 Xu hướng doanh thu 12 tháng gần nhất</h6>
    </div>
    <div class="card-body">
        ...
        <div class="card border-0 shadow-sm h-100">
            ...
            <div class="card-body p-0">
                <table class="table table-sm table-hover mb-0" style="font-size:.85rem">
                    ...
                    <tr>
                        <td class="ps-3"><?= htmlspecialchars($r['company_name']) ?></td>
                        <td class="text-center"><?= $r['trips'] ?></td>
                        <td class="text-end fw-bold text-success"><?= formatMoney((float)$r['revenue']) ?></td>
                    </tr>
                    <?php endforeach; ?>
        ...
        <div class="card border-0 shadow-sm h-100">
            ...
            <div class="card-body p-0">
                <table class="table table-sm table-hover mb-0" style="font-size:.85rem">
                    ...
                    <tr>
                        <td class="ps-3"><?= htmlspecialchars($r['full_name']) ?></td>
                        <td class="text-center"><?= $r['trips'] ?></td>
                        <td class="text-end"><?= number_format((float)$r['km'], 0) ?></td>
                        <td class="text-end fw-bold text-primary"><?= formatMoney((float)$r['revenue']) ?></td>
                    </tr>
                    <?php endforeach; ?>
                    </tbody>
                </table>
...
<div class="card border-0 shadow-sm mb-4">
    ...
    <div class="card-body p-0">
        <table class="table table-sm mb-0" style="font-size:.85rem">
            ...
            ?>
            <tr>
                <td class="ps-3"><span class="badge bg-<?= $sm['class'] ?>"><?= $sm['label'] ?></span></td>
                <td class="text-center"><?= number_format((int)$r['cnt']) ?></td>
                <td class="text-end"><?= formatMoney((float)$r['amt']) ?></td>
            </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
...
<div class="row g-3 mb-4">
    ...
    <div class="col-md-3">
        <div class="card border-0 shadow-sm text-center p-3 border-start border-info border-4">
            <div class="fs-4 fw-bold text-info"><?= number_format($revTotalKm + array_sum(array_column($revenueData,'total_km')), 0) ?> km</div>
...
<div class="card border-0 shadow-sm mb-4">
    ...
    <div class="card-body p-0">
        <table class="table table-hover align-middle mb-0" style="font-size:.85rem">
            ...
            <tr>
                <td class="ps-3">
                    <div class="fw-semibold"><?= htmlspecialchars($r['short_name'] ?: $r['company_name']) ?></div>
                    <small class="text-muted"><?= htmlspecialchars($r['customer_code'] ?? '') ?></small>
                </td>
                <td class="small">
                    <?= date('d/m/Y', strtotime($r['period_from'])) ?><br>
                    <span class="text-muted">→ <?= date('d/m/Y', strtotime($r['period_to'])) ?></span>
                </td>
...
<div class="card border-0 shadow-sm mb-4">
    ...
    <div class="card-body p-0">
        ...
        <table class="table table-hover align-middle mb-0" style="font-size:.83rem">
            <thead class="table-light">
                <tr>
                    <th class="ps-3">Ngày</th><th>Mã chuyến</th><th>Khách hàng</th>
                    <th>Lái xe</th><th>Biển số</th>
                    <th class="text-end">KM</th><th class="text-end">Cầu đường</th>
                    <th class="text-end">Thành tiền</th><th class="text-center">Trạng thái</th>
                </tr>
            </thead>
            <tbody>
...
<div class="card border-0 shadow-sm mb-4">
    ...
    <table class="table table-hover align-middle mb-0" style="font-size:.83rem">
        <thead class="table-light">
            <tr>
                <th class="ps-3">Ngày</th><th>Biển số</th><th>Lái xe</th>
                <th class="text-end">Lít</th><th class="text-end">Đơn giá</th>
                <th class="text-end">Tiền</th><th class="text-end">KM trước</th>
                <th class="text-end">KM sau</th><th class="text-end">KM chạy</th>
                <th class="text-end">L/100km</th><th>Trạm</th>
            </tr>
        </thead>
        <tbody>
        ...
        <tr>
            ...
            <td class="text-end <?= ($r['fuel_efficiency'] ?? 0) > 35 ? 'text-danger' : '' ?>">
                <?= $r['fuel_efficiency'] ? number_format((float)$r['fuel_efficiency'],1) : '—' ?>
            </td>
            <td class="small text-muted"><?= htmlspecialchars($r['station_name'] ?? '') ?></td>
        </tr>
        <?php endforeach; ?>
        <?php if (empty($costData['fuel'])): ?>
        <tr><td colspan="11" class="text-center text-muted py-3">Không có dữ liệu</td></tr>
        <?php endif; ?>
        </tbody>
        <?php if (!empty($costData['fuel'])): ?>
        <tfoot class="table-light fw-bold">
            <tr>
                ...
                <td></td>
                <td class="text-end text-warning"><?= formatMoney($costData['_fuel_total']) ?></td>
                <td colspan="3" class="text-end text-muted"><?= number_format($costData['_fuel_km'],0) ?> km</td>
                <td colspan="2"></td>
            </tr>
        </tfoot>
        <?php endif; ?>
...
<div class="card border-0 shadow-sm mb-4">
    ...
    <table class="table table-hover align-middle mb-0" style="font-size:.83rem">
        <thead class="table-light">
            <tr>
                <th class="ps-3">Ngày</th><th>Biển số</th><th>Loại BT</th>
                <th>Mô tả</th><th class="text-end">ODO</th>
                <th class="text-end fw-bold">Chi phí</th><th>Gara</th>
                <th>Lần BT kế</th>
            </tr>
        </thead>
        <tbody>
        ...
        <tr>
            ...
            <td class="text-end small"><?= $r['mileage'] ? number_format((float)$r['mileage'],0).' km' : '—' ?></td>
        ...
        <tfoot class="table-light fw-bold">
            <tr>
                <td colspan="5" class="ps-3 text-end">Tổng (<?= count($costData['maintenance']) ?> lần):</td>
                <td class="text-end text-danger"><?= formatMoney($costData['_maint_total']) ?></td>
                <td colspan="2"></td>
            </tr>
        </tfoot>
        <?php endif; ?>
    ...
    <table class="table table-hover align-middle mb-0" style="font-size:.85rem">
        ...
        <tr>
            ...
            <td class="text-center">
                <span class="badge bg-<?= $margin >= 0 ? 'success' : 'danger' ?>"><?= $margin ?>%</span>
            </td>
            <td class="text-center">
                <?= $r['is_locked']
                    ? '<span class="badge bg-success">🔒 Đã chốt</span>'
                    : '<span class="badge bg-warning text-dark">⏳ Tạm tính</span>' ?>
            </td>
        </tr>
        <?php endforeach; ?>
        </tbody>
    ...
    <table class="table table-hover align-middle mb-0" style="font-size:.83rem">
        <thead class="table-dark">
            <tr>
                ...
                <th class="text-end">KM</th>
                <th class="text-end">Doanh thu</th>
                <th class="text-end">Avg/chuyến</th>
                <th class="text-center">% DT</th>
                <th>Chuyến đầu</th><th>Chuyến cuối</th>
            </tr>
        </thead>
        <tbody>
        ...
        <tr>
            <td class="ps-3">
                <div class="fw-semibold"><?= htmlspecialchars($r['short_name'] ?: $r['company_name']) ?></div>
                <small class="text-muted"><?= $r['customer_code'] ?> | <?= htmlspecialchars($r['contact_name'] ?? '') ?></small>
            </td>
            ...
            <td class="text-center">
                <div class="progress" style="height:16px;min-width:60px">
                    <div class="progress-bar bg-success" style="width:<?= min(100,$r['revenue_pct']) ?>%">
                        <?= $r['revenue_pct'] ?>%
                    </div>
                </div>
            </td>
...
<div class="card border-0 shadow-sm mb-4">
    ...
    <table class="table table-hover align-middle mb-0" style="font-size:.83rem">
        <thead class="table-dark">
            <tr>
                <th class="ps-3">Biển số</th><th>Hãng / Model</th>
                <th class="text-center">Chuyến</th>
                <th class="text-end">KM</th>
                <th class="text-end">Doanh thu</th>
                <th class="text-end">NL (₫)</th>
...
<div class="card border-0 shadow-sm mb-4">
    ...
    <table class="table table-hover align-middle mb-0" style="font-size:.83rem">
        <thead class="table-dark">
            <tr>
                <th class="ps-3">Lái xe</th><th>SĐT</th><th>GPLX</th>
                <th class="text-center">Chuyến</th>
                <th class="text-end">KM</th>
                <th class="text-end">Doanh thu</th>
        ...
        <tr>
            ...
            <td class="text-center">
                <?= $r['faults'] > 0
                    ? '<span class="badge bg-danger">'.$r['faults'].'</span>'
...
<div class="card border-0 shadow-sm mb-4">
    ...
    <table class="table table-sm table-hover mb-0" style="font-size:.83rem">
        ...
        <tr>
            ...
            <td class="text-end <?= ($r['avg_l100km']??0) > 35 ? 'text-danger' : '' ?>">
                <?= $r['avg_l100km'] ? number_format((float)$r['avg_l100km'],1) : '—' ?>
            </td>
        </tr>
        <?php endforeach; ?>
        </tbody>
...
<div class="card border-0 shadow-sm mb-4">
    ...
    <table class="table table-sm table-hover align-middle mb-0" style="font-size:.82rem">
        <thead class="table-light">
            <tr>
                <th class="ps-3">Ngày</th><th>Biển số</th><th>Lái xe</th>
                <th class="text-end">Lít</th><th class="text-end">Đơn giá</th><th class="text-end">Tiền</th>
                <th class="text-end">KM trước</th><th class="text-end">KM sau</th>
                <th class="text-end">L/100km</th><th>Trạm</th>
            </tr>
        </thead>
        <tbody>
        ...
        <tr><td colspan="10" class="text-center text-muted py-3">Không có dữ liệu</td></tr>
        <?php endif; ?>
        </tbody>
    </table>
    </div>
