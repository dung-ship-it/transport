...
if ($tab === 'overview') {
    ...
    $tripsRevenue = (float)safeCol($pdo, "
        SELECT COALESCE(SUM(COALESCE(total_amount,0)), 0) FROM trips
    ...
    $overview['by_status'] = safeQuery($pdo, "
        SELECT status, COUNT(*) AS cnt, COALESCE(SUM(COALESCE(total_amount,0)),0) AS amt
    ...
    $overview['monthly'] = safeQuery($pdo, "
        WITH months AS (
            ...
            FROM trips WHERE trip_date >= CURRENT_DATE - INTERVAL '12 months'
        ),
        td AS (
    ...
    $overview['top_customers'] = safeQuery($pdo, "
        SELECT c.id, c.company_name, COUNT(t.id) AS trips,
               COALESCE(SUM(COALESCE(t.total_amount,0)),0) AS revenue
    ...
    // Override doanh thu top KH từ kỳ locked nếu có
    foreach ($overview['top_customers'] as &$cRow) {
        $lr = (float)safeCol($pdo, "
            SELECT COALESCE(SUM(si.total_amount),0)
            FROM statement_items si
            JOIN statement_periods sp ON si.period_id = sp.id
            WHERE sp.status='locked' AND si.customer_id=?
              AND sp.period_from <= ? AND sp.period_to >= ?
        ", [$cRow['id'], $dateTo, $dateFrom]);
        if ($lr > 0) $cRow['revenue'] = $lr;
    }
    unset($cRow);
...
if ($tab === 'revenue') {
    ...
    $sqlItems   = "
        ...
        WHERE sp.status = 'locked'
          AND sp.period_from <= ? AND sp.period_to >= ?
    ";
    $itemParams = [$dateTo, $dateFrom];
    if ($filterCustomer) { $sqlItems .= " AND si.customer_id = ?"; $itemParams[] = $filterCustomer; }
    ...
    $sqlUT    = "
        SELECT t.trip_date, t.trip_code,
               t.total_km, COALESCE(t.total_amount,0) AS total_amount,
               COALESCE(t.toll_fee,0) AS toll_fee, t.status,
...
if ($tab === 'cost') {
    ...
    $sqlMaint    = "
        SELECT ml.log_date AS maintenance_date,
               ...
               ml.total_cost AS cost, ml.parts_cost, ml.labor_cost,
               ml.odometer_km AS mileage, ml.garage_name,
               ml.next_maintenance_date, ml.note AS notes,
               ml.invoice_number, ml.status, v.plate_number
        FROM maintenance_logs ml
        JOIN vehicles v ON ml.vehicle_id = v.id
        WHERE ml.log_date BETWEEN ? AND ? AND ml.status = 'completed'
    ";
...
if ($tab === 'customer') {
    $customerReport = safeQuery($pdo, "
        SELECT c.id, c.company_name, c.short_name, c.customer_code,
               ...
               COALESCE(SUM(COALESCE(t.total_amount,0)),0)  AS total_revenue,
               COALESCE(AVG(COALESCE(t.total_amount,0)),0)  AS avg_per_trip,
    ...
    foreach ($customerReport as &$row) {
        $lr = (float)safeCol($pdo, "
            ...
            WHERE sp.status = 'locked' AND si.customer_id = ?
              AND sp.period_from <= ? AND sp.period_to >= ?
        ", [$row['id'], $dateTo, $dateFrom]);
        if ($lr > 0) $row['total_revenue'] = $lr;
        $row['fuel_cost'] = 0;
    }
    unset($row);
...
<div class="card border-0 shadow-sm mb-4">
    ...
    <div class="card-body p-0">
        <table class="table table-sm mb-0" style="font-size:.85rem">
            ...
            <?php
            $statusMap = [
                'completed'   => ['label'=>'Hoàn thành', 'class'=>'primary'],
                'confirmed'   => ['label'=>'Đã duyệt',   'class'=>'success'],
                'rejected'    => ['label'=>'Từ chối',    'class'=>'danger'],
                'in_progress' => ['label'=>'Đang chạy',  'class'=>'warning'],
                'scheduled'   => ['label'=>'Chờ',        'class'=>'secondary'],
            ];
...
<div class="card border-0 shadow-sm mb-4">
    ...
    <table class="table table-hover align-middle mb-0" style="font-size:.83rem">
        ...
        <tfoot class="table-light fw-bold">
            <tr>
                <td colspan="6" class="ps-3 text-end">Tổng (<?= count($costData['maintenance']) ?> lần):</td>
