...
if ($day >= 26) {
    ...
    // Chưa tới ngày 26: đang trong kỳ hiện tại (26 tháng trước → 25 tháng này)
    $periodFrom = (clone $today)
        ->setDate((int)$today->format('Y'), (int)$today->format('m'), 26)
        ->modify('-1 month');   // ✅ chỉ lùi 1 tháng
    $periodTo = (clone $today)
        ->setDate((int)$today->format('Y'), (int)$today->format('m'), 25);
        // ✅ không modify gì — giữ nguyên tháng này
}
...
<div class="col-md-2">
    ...
    </div><!-- ← Đóng card bộ lọc -->

<!-- ✅ Nút chốt kỳ đặt NGOÀI form, sau card bộ lọc -->
...
<div class="d-flex align-items-center gap-3 mb-3">
    ...
    ?>
    <?php if ($lockedPeriod): ?>
    <span class="badge bg-<?= $lockedPeriod['status'] === 'locked' ? 'success' : 'warning' ?> fs-6">
        <?= $lockedPeriod['status'] === 'locked' ? '🔒 Đã chốt' : '📝 Bản nháp' ?>
        <?php if ($lockedPeriod['locked_at']): ?>
        <small class="opacity-75">
            (<?= date('d/m H:i', strtotime($lockedPeriod['locked_at'])) ?>)
        </small>
        <?php endif; ?>
    </span>
    <?php endif; ?>
