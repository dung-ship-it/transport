...
if ($day >= 26) {
    ...
    $periodTo   = (clone $today)->setDate((int)$today->format('Y'), (int)$today->format('m'), 25)->modify('+1 month');
} else {
    $periodFrom = (clone $today)->setDate((int)$today->format('Y'), (int)$today->format('m'), 26)->modify('-1 month');
...
// ── Xử lý POST chốt kỳ (fallback không dùng JS) ─────────────
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    $pDateFrom = $_POST['date_from'] ?? $dateFrom;
    $pDateTo   = $_POST['date_to']   ?? $dateTo;

    if ($_POST['action'] === 'save_draft') {
        $chk = $pdo->prepare("SELECT id FROM statement_periods WHERE period_from = ? AND period_to = ?");
        $chk->execute([$pDateFrom, $pDateTo]);
        $existing = $chk->fetch();
        if (!$existing) {
            $pdo->prepare("
                INSERT INTO statement_periods (period_from, period_to, status, created_by, created_at)
                VALUES (?, ?, 'draft', ?, NOW())
            ")->execute([$pDateFrom, $pDateTo, $user['id']]);
        }
        $_SESSION['flash'] = ['type' => 'info', 'msg' => '📝 Đã lưu bản nháp!'];
        header("Location: index.php?date_from=$pDateFrom&date_to=$pDateTo");
        exit;
    }
}
...
foreach ($allTrips as $t) {
    ...
    if (!isset($statements[$cid]['vehicles'][$plate])) {
        $statements[$cid]['vehicles'][$plate] = [
            ...
            'combo_monthly_price'   => $t['combo_monthly_price'] ?? 0,
            'combo_km_limit'        => $t['combo_km_limit'] ?? 0,
            'over_km_price'         => $t['over_km_price'] ?? 0,
            'standard_price_per_km' => $t['standard_price_per_km'] ?? 0,
            'toll_included'         => $t['toll_included'] ?? false,
            'holiday_surcharge'     => $t['holiday_surcharge'] ?? 0,
            'sunday_surcharge'      => $t['sunday_surcharge']  ?? 0,
            'has_rule'              => (bool)($t['rule_id'] ?? false),
            'trip_count'            => 0,
            'total_km'              => 0,
            'total_toll'            => 0,
            'sunday_km'             => 0,
    ...
    $veh['trip_count']++;
    $veh['total_km']   += (float)($t['total_km'] ?? 0);
    $veh['total_toll'] += (float)($t['toll_fee'] ?? 0);
    if (!empty($t['is_sunday'])) {
        $veh['sunday_km']    += (float)($t['total_km'] ?? 0);
        $veh['sunday_trips'] += 1;
    }
    $veh['trips'][] = $t;
    ...
    $statements[$cid]['total_km']   += (float)($t['total_km'] ?? 0);
    $statements[$cid]['total_toll'] += (float)($t['toll_fee'] ?? 0);
    unset($veh);
}
...
$chkLocked = $pdo->prepare("
    SELECT id, status, locked_at FROM statement_periods
    WHERE period_from = ? AND period_to = ?
");
$chkLocked->execute([$dateFrom, $dateTo]);
$lockedPeriod = $chkLocked->fetch();
...
<div class="container-fluid py-4">
    ...
    <div class="d-flex align-items-center gap-3 mb-4 flex-wrap" id="lockBar">
        ...
        <a href="../reports/index.php?tab=revenue&date_from=<?= urlencode($dateFrom) ?>&date_to=<?= urlencode($dateTo) ?>"
           class="btn btn-sm btn-outline-success">
            <i class="fas fa-chart-bar me-1"></i>Xem báo cáo doanh thu
        </a>
        <button class="btn btn-sm btn-outline-warning"
                onclick="doUnlock('<?= htmlspecialchars($dateFrom) ?>','<?= htmlspecialchars($dateTo) ?>')">
        ...
        <!-- Chốt kỳ — gọi JS fetch → lock_period.php → redirect reports -->
        <button type="button" class="btn btn-success fw-bold" onclick="doLock()">
            <i class="fas fa-lock me-1"></i>Chốt kỳ
            <span class="badge bg-white text-success ms-1"><?= $grandTrips ?> chuyến</span>
        </button>
        ...
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <div class="modal-content border-0 shadow">
                <div class="modal-body text-center py-4">
                    ...
                    <div id="lockModalMsg" class="fw-semibold">Đang chốt kỳ...</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-6 col-md-3">
            <div class="card border-0 shadow-sm text-center p-3 border-start border-success border-4">
                <div class="fs-2 fw-bold text-success"><?= number_format($grandKm, 0) ?> km</div>
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-header bg-white border-bottom py-2 d-flex justify-content-between align-items-center">
            ...
            <small class="text-muted">
                ...
                <?php if ($lockedPeriod && $lockedPeriod['status'] === 'locked'): ?>
                <span class="badge bg-success ms-2">🔒 Đã chốt</span>
                <?php endif; ?>
            </small>
        </div>
        <div class="card-body p-0">
            ...
            <table class="table table-hover align-middle mb-0" style="font-size:0.85rem">
                ...
                <tr class="<?= !$stmt['has_price'] ? 'table-warning bg-opacity-25' : '' ?>">
                    <td class="ps-3">
                        ...
                        <div class="text-muted small">
                            <span class="badge bg-secondary"><?= htmlspecialchars($stmt['customer_code']) ?></span>
                    ...
                    <td class="text-center"><span class="badge bg-info"><?= count($stmt['vehicles']) ?></span></td>
                    <td class="text-center"><?= $stmt['trip_count'] ?></td>
                    <td class="text-center">
                        <?php
                        $pct = $stmt['trip_count'] > 0
                            ? round($stmt['confirmed_count'] / $stmt['trip_count'] * 100) : 0;
                        ?>
                        <span class="badge bg-<?= $pct==100?'success':($pct>0?'warning':'secondary') ?>">
                            <?= $stmt['confirmed_count'] ?>/<?= $stmt['trip_count'] ?>
                        </span>
                    </td>
                    <td class="text-end fw-semibold"><?= number_format($stmt['total_km'], 0) ?> km</td>
                    <td class="text-end"><?= number_format($stmt['total_toll'], 0, '.', ',') ?> ₫</td>
                    <td class="text-end small text-muted"><?= htmlspecialchars($stmt['pb_name']) ?></td>
                    <td class="text-end fw-bold fs-6 <?= $stmt['has_price'] ? 'text-success' : 'text-muted' ?>">
                        <?= $stmt['has_price']
                            ? number_format($stmt['total_amount'], 0, '.', ',') . ' ₫'
                            : '—' ?>
                    </td>
                    <td class="text-center">
                        <a href="?date_from=<?= urlencode($dateFrom) ?>&date_to=<?= urlencode($dateTo) ?>&customer_id=<?= $cid ?>#stmt-<?= $cid ?>"
                           class="btn btn-sm btn-outline-primary">
                            <i class="fas fa-eye me-1"></i>Chi tiết
                        </a>
                        <a href="print_statement_billing.php?customer_id=<?= $cid ?>&date_from=<?= urlencode($dateFrom) ?>&date_to=<?= urlencode($dateTo) ?>"
                ...
                <tfoot class="table-dark">
                    <tr>
                        <td class="ps-3 fw-bold">TỔNG CỘNG (<?= count($statements) ?> khách hàng)</td>
                        <td></td>
                        <td class="text-center fw-bold"><?= $grandTrips ?></td>
                        <td></td>
                        <td class="text-end fw-bold"><?= number_format($grandKm, 0) ?> km</td>
                        <td class="text-end fw-bold"><?= number_format($grandToll, 0, '.', ',') ?> ₫</td>
                        <td></td>
    ...
    <?php foreach ($statements as $cid => $stmt): ?>
    <div class="card border-0 shadow-sm mb-4" id="stmt-<?= $cid ?>">
        <div class="card-header py-2 d-flex justify-content-between align-items-center flex-wrap gap-2"
            ...
            <div>
                ...
                <span class="badge bg-light text-dark ms-2"><?= htmlspecialchars($stmt['customer_code']) ?></span>
                <?php if ($stmt['tax_code']): ?>
                <span class="small opacity-75 ms-2">MST: <?= htmlspecialchars($stmt['tax_code']) ?></span>
                <?php endif; ?>
            </div>
            <div class="d-flex gap-2 align-items-center flex-wrap">
                <?php if ($stmt['has_price']): ?>
                <span class="fw-bold fs-6">💰 <?= number_format($stmt['total_amount'], 0, '.', ',') ?> ₫</span>
                <?php else: ?>
                <span class="badge bg-warning text-dark">⚠️ Chưa có bảng giá</span>
                <?php endif; ?>
                <a href="print_statement_billing.php?customer_id=<?= $cid ?>&date_from=<?= urlencode($dateFrom) ?>&date_to=<?= urlencode($dateTo) ?>"
        ...
        <div class="card-body p-0">
            ...
            <div class="px-3 py-2 bg-light border-bottom small text-muted">
                ...
                Bảng giá: <strong class="text-primary"><?= htmlspecialchars($stmt['pb_name']) ?></strong>
                ...
                <table class="table table-bordered align-middle mb-0" style="font-size:0.82rem">
                    <thead style="background:#e8f4fd">
                        <tr class="text-center">
                            ...
                            <th rowspan="2">KM COMBO/tháng</th>
                            <th rowspan="2">Số chuyến</th>
                            <th rowspan="2">Tổng KM thực tế</th>
                        ...
                        <tr class="text-center">
                            <th class="table-warning small">Tiền COMBO/KM cơ bản</th>
                            <th class="table-warning small">Tiền quá KM</th>
                            <th class="table-warning small">T��ng cơ bản</th>
                        </tr>
                    </thead>
                    <tbody>
                    ...
                    <tr class="<?= !$veh['has_rule'] ? 'table-warning bg-opacity-25' : '' ?>">
                        <td class="ps-3 fw-bold text-primary">
                            ...
                            <?php if (!$veh['has_rule']): ?>
                            <span class="badge bg-warning text-dark ms-1" style="font-size:0.65rem">Chưa có giá</span>
                            <?php endif; ?>
                        </td>
                        <td class="text-center text-muted small"><?= $veh['capacity'] ? htmlspecialchars((string)$veh['capacity']).' tấn' : '—' ?></td>
                        <td class="text-center">
                        ...
                        <td class="text-end small">
                            <?php if ($veh['pricing_mode'] === 'combo'): ?>
                            <span class="fw-bold text-success"><?= number_format((float)$veh['combo_monthly_price'], 0, '.', ',') ?> ₫</span>
                            <div class="text-muted" style="font-size:0.7rem">/tháng</div>
                            <?php elseif ($veh['pricing_mode'] === 'standard'): ?>
                            <span class="fw-bold text-info"><?= number_format((float)$veh['standard_price_per_km'], 0, '.', ',') ?></span>
                            <div class="text-muted" style="font-size:0.7rem">₫/km</div>
                            <?php else: ?>—<?php endif; ?>
                        </td>
                        <td class="text-center small"><?= $veh['combo_km_limit'] ? number_format((float)$veh['combo_km_limit'], 0).' km' : '—' ?></td>
                        <td class="text-center"><?= $veh['trip_count'] ?></td>
                        <td class="text-end fw-semibold text-primary"><?= number_format($veh['total_km'], 0) ?> km</td>
                        <td class="text-end <?= ($veh['over_km'] ?? 0) > 0 ? 'text-danger fw-bold' : 'text-muted' ?>">
                            <?= ($veh['over_km'] ?? 0) > 0 ? number_format((float)$veh['over_km'], 0).' km' : '—' ?>
                        </td>
                        <td class="text-end table-warning">
                            <?php if ($veh['has_rule']): ?>
                                <?= $veh['pricing_mode'] === 'combo'
                                    ? number_format((float)$veh['combo_monthly_price'], 0, '.', ',').' ₫'
                                    : number_format($veh['total_km'] * (float)$veh['standard_price_per_km'], 0, '.', ',').' ₫' ?>
                            <?php else: ?>—<?php endif; ?>
                        </td>
                        <td class="text-end table-warning">
                            <?= ($veh['over_amount'] ?? 0) > 0 ? number_format((float)$veh['over_amount'], 0, '.', ',').' ₫' : '—' ?>
                        </td>
                        <td class="text-end table-warning fw-semibold">
                            <?= $veh['has_rule'] ? number_format((float)$veh['amount_base'], 0, '.', ',').' ₫' : '—' ?>
                        </td>
                        <td class="text-end">
                            <?php if ($veh['toll_included']): ?>
                            <span class="text-muted small">Đã bao gồm</span>
                            <?php elseif ($veh['amount_toll'] > 0): ?>
                            <span class="text-warning fw-semibold"><?= number_format((float)$veh['amount_toll'], 0, '.', ',') ?> ₫</span>
                            <?php else: ?>—<?php endif; ?>
                        </td>
                        <td class="text-end">
                            <?= $veh['amount_surcharge'] > 0
                                ? '<span class="text-info fw-semibold">'.number_format((float)$veh['amount_surcharge'], 0, '.', ',').' ₫</span>'
                                  .' <small class="text-muted">('.$veh['sunday_trips'].' CN)</small>'
                                : '—' ?>
                        </td>
                        <td class="text-end fw-bold table-success fs-6">
                            <?= $veh['has_rule'] ? number_format((float)$veh['amount_total'], 0, '.', ',').' ₫' : '<span class="text-muted">—</span>' ?>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                    </tbody>
                    <tfoot>
                        <tr class="fw-bold" style="background:#e8f4fd">
                            <td class="ps-3" colspan="5">TỔNG CỘNG (<?= count($stmt['vehicles']) ?> xe, <?= $stmt['trip_count'] ?> chuyến)</td>
                            <td class="text-center"><?= $stmt['trip_count'] ?></td>
                            <td class="text-end text-primary"><?= number_format($stmt['total_km'], 0) ?> km</td>
                            <td></td>
                            <td colspan="3"></td>
                            <td class="text-end"><?= number_format($stmt['total_toll'], 0, '.', ',') ?> ₫</td>
                            <td></td>
                            <td class="text-end text-success fs-6">
                                <?= $stmt['has_price'] ? number_format($stmt['total_amount'], 0, '.', ',').' ₫' : '—' ?>
                            </td>
                        </tr>
                    </tfoot>
                </table>
            ...
            <!-- Chi tiết chuyến (collapsible) -->
            <div class="px-3 py-2">
                <button class="btn btn-sm btn-outline-secondary" type="button"
                        data-bs-toggle="collapse" data-bs-target="#trips-<?= $cid ?>" aria-expanded="false">
                    <i class="fas fa-list me-1"></i>Xem chi tiết <?= $stmt['trip_count'] ?> chuyến
                    ...
                    <table class="table table-sm table-hover align-middle mb-0" style="font-size:0.78rem">
                        ...
                        foreach ($stmt['vehicles'] as $plate => $veh):
                            foreach ($veh['trips'] as $t):
                                $i++;
                                $statusCls = ['completed'=>'primary','confirmed'=>'success','rejected'=>'danger'][$t['status']] ?? 'secondary';
                                $statusLbl = ['completed'=>'✅ Hoàn thành','confirmed'=>'👍 Đã duyệt','rejected'=>'❌ Từ chối'][$t['status']] ?? htmlspecialchars($t['status']);
                                $from = htmlspecialchars($t['pickup_location']  ?? $t['route_from'] ?? '—');
                                $to   = htmlspecialchars($t['dropoff_location'] ?? $t['route_to']   ?? '—');
                        ?>
                        <tr class="<?= $t['status']==='confirmed' ? 'table-success bg-opacity-10' : ($t['status']==='rejected' ? 'table-danger bg-opacity-10' : '') ?>">
                            <td class="ps-3 text-muted"><?= $i ?></td>
                            <td class="text-nowrap">
                                <?= date('d/m/Y', strtotime($t['trip_date'])) ?>
                                <?php if (!empty($t['is_sunday'])): ?><span class="badge bg-warning" style="font-size:0.6rem">CN</span><?php endif; ?>
                            </td>
                            <td><code><?= htmlspecialchars($t['trip_code'] ?? '') ?></code></td>
                            <td class="fw-bold text-primary"><?= htmlspecialchars($t['plate_number']) ?></td>
                            ...
                            <td class="text-end"><?= isset($t['odometer_start']) && $t['odometer_start'] ? number_format($t['odometer_start'], 0) : '—' ?></td>
                            <td class="text-end"><?= isset($t['odometer_end']) && $t['odometer_end'] ? number_format($t['odometer_end'], 0) : '—' ?></td>
                            <td class="text-end fw-bold text-primary"><?= isset($t['total_km']) && $t['total_km'] ? number_format((float)$t['total_km'], 0).' km' : '—' ?></td>
                            <td class="text-end"><?= isset($t['toll_fee']) && $t['toll_fee'] ? number_format((float)$t['toll_fee'], 0, '.', ',').' ₫' : '—' ?></td>
                            <td class="text-muted"><?= htmlspecialchars($t['note'] ?? '') ?></td>
                            <td class="text-center"><span class="badge bg-<?= $statusCls ?>"><?= $statusLbl ?></span></td>
                        </tr>
                        <?php endforeach; endforeach; ?>
                        </tbody>
                        <tfoot class="table-light fw-bold">
                            <tr>
                                <td colspan="8" class="ps-3 text-end">Tổng (<?= $stmt['trip_count'] ?> chuyến):</td>
                                <td class="text-end text-primary"><?= number_format($stmt['total_km'], 0) ?> km</td>
                                <td class="text-end"><?= number_format($stmt['total_toll'], 0, '.', ',') ?> ₫</td>
                                <td colspan="2"></td>
                            </tr>
    ...
    <div class="card border-0 shadow-sm p-5 text-center">
        ...
        <p class="text-muted">Chọn kỳ thời gian và nhấn <strong>Tính toán</strong> để xem bảng kê công nợ</p>
        ...
        <div class="mt-3">
            <a href="history.php" class="btn btn-outline-secondary btn-sm">
                <i class="fas fa-history me-1"></i>Xem lịch sử chốt
            </a>
        </div>
    </div>
    <?php endif; ?>
...
<!-- ── JS chốt kỳ ── -->
<script>
// Dữ liệu statements gửi kèm lên lock_period.php
const statementsData = <?= json_encode(array_values($statements), JSON_UNESCAPED_UNICODE) ?>;

async function doLock() {
    const df = '<?= addslashes($dateFrom) ?>';
    const dt = '<?= addslashes($dateTo) ?>';
    const dfFmt = '<?= date('d/m/Y', strtotime($dateFrom)) ?>';
    const dtFmt = '<?= date('d/m/Y', strtotime($dateTo)) ?>';

    if (!confirm(
        'Xác nhận CHỐT KỲ từ ' + dfFmt + ' đến ' + dtFmt + '?\n\n' +
        '• Số liệu doanh thu được ghi nhận chính thức\n' +
        '• Các chuyến "completed" chuyển thành "confirmed"\n' +
        '• Hệ thống tự chuyển sang Báo Cáo Tổng Hợp\n\n' +
        'Sau khi chốt có thể mở lại nếu cần.'
    )) return;

    const modal = new bootstrap.Modal(document.getElementById('lockModal'));
    modal.show();
    document.getElementById('lockModalMsg').innerHTML = '<span class="text-muted">Đang chốt kỳ, vui lòng chờ...</span>';

    try {
        const res = await fetch('lock_period.php', {
            method: 'POST',
        ...
        if (data.ok) {
            document.getElementById('lockModalMsg').innerHTML =
                '<i class="fas fa-check-circle text-success fa-2x mb-2 d-block"></i>' +
                '<strong>' + data.msg + '</strong>' +
                '<br><small class="text-muted">Đang chuyển sang báo cáo doanh thu...</small>';
...
async function doUnlock(dateFrom, dateTo) {
    if (!confirm('Xác nhận MỞ LẠI kỳ này để chỉnh sửa?\nDữ liệu báo cáo sẽ ở trạng thái nháp.')) return;
    try {
        const res = await fetch('lock_period.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'unlock', date_from: dateFrom, date_to: dateTo })
        });
        const data = await res.json();
        if (data.ok) {
            alert(data.msg);
...
<style>
@media print {
    .sidebar, .topbar, nav.navbar, form, .btn, .alert, #lockBar, #lockModal { display:none !important; }
    .main-content { margin:0 !important; padding:0 !important; }
    .container-fluid { padding:5mm !important; }
    .collapse { display:block !important; }
    .card { border:1px solid #ccc !important; box-shadow:none !important; margin-bottom:10mm !important; }
    .card-header { -webkit-print-color-adjust:exact; print-color-adjust:exact; }
    table { font-size:8pt !important; }
    .table-success, .table-warning, .table-dark { -webkit-print-color-adjust:exact; print-color-adjust:exact; }
}
</style>
