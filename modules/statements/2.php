...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'lock_statement') {
        ...
        if ($existing && $existing['status'] === 'locked') {
            $_SESSION['flash'] = ['type' => 'warning', 'msg' => '⚠️ Kỳ này đã được chốt rồi!'];
        } elseif ($existing) {
            // Cập nhật thành locked
            $pdo->prepare("
                UPDATE statement_periods
                SET status = 'locked', locked_at = NOW(), locked_by = ?
                WHERE id = ?
            ")->execute([$user['id'], $existing['id']]);
            $_SESSION['flash'] = ['type' => 'success', 'msg' => '🔒 Đã chốt kỳ thành công!'];
        } else {
            // Tạo mới
            $pdo->prepare("
                INSERT INTO statement_periods (period_from, period_to, status, locked_at, locked_by, created_by, created_at)
                VALUES (?, ?, 'locked', NOW(), ?, ?, NOW())
            ")->execute([$pDateFrom, $pDateTo, $user['id'], $user['id']]);
            $_SESSION['flash'] = ['type' => 'success', 'msg' => '🔒 Đã chốt kỳ thành công!'];
        }
...
<div class="container-fluid py-4">
    ...
    <div class="d-flex align-items-center gap-3 mb-4">
        ...
        <!-- Form chốt kỳ -->
        <form method="POST" class="d-inline"
              onsubmit="return confirm('Xác nhận CHỐT KỲ từ <?= date('d/m/Y', strtotime($dateFrom)) ?> đến <?= date('d/m/Y', strtotime($dateTo)) ?>?\nSau khi chốt sẽ không thể thay đổi.')">
            <input type="hidden" name="action"    value="lock_statement">
            <input type="hidden" name="date_from" value="<?= $dateFrom ?>">
            <input type="hidden" name="date_to"   value="<?= $dateTo ?>">
            ...
            <div class="table-responsive">
                <table class="table table-bordered align-middle mb-0" style="font-size:0.82rem">
                    ...
                    <tr class="<?= !$veh['has_rule'] ? 'table-warning bg-opacity-25' : '' ?>">
                        ...
                        <td class="text-center text-muted small"><?= $veh['capacity'] ? $veh['capacity'].' tấn' : '—' ?></td>
                        <td class="text-center">
                        ...
                        <td class="text-end small">
                            <?php if ($veh['pricing_mode'] === 'combo'): ?>
                            <span class="fw-bold text-success"><?= number_format($veh['combo_monthly_price'] ?? 0, 0, '.', ',') ?> ₫</span>
                            <div class="text-muted" style="font-size:0.7rem">/tháng</div>
                            <?php elseif ($veh['pricing_mode'] === 'standard'): ?>
                            <span class="fw-bold text-info"><?= number_format($veh['standard_price_per_km'] ?? 0, 0, '.', ',') ?></span>
                            <div class="text-muted" style="font-size:0.7rem">₫/km</div>
                            <?php else: ?>—<?php endif; ?>
                        </td>
                        <td class="text-center small"><?= $veh['combo_km_limit'] ? number_format($veh['combo_km_limit'], 0).' km' : '—' ?></td>
                        ...
                        <td class="text-end table-warning">
                            <?php if ($veh['has_rule']): ?>
                                <?= $veh['pricing_mode'] === 'combo'
                                    ...
                                    : number_format($veh['total_km'] * ($veh['standard_price_per_km'] ?? 0), 0, '.', ',').' ₫' ?>
                            <?php else: ?>—<?php endif; ?>
                        </td>
                        <td class="text-end table-warning">
                            <?= ($veh['over_amount'] ?? 0) > 0 ? number_format($veh['over_amount'], 0, '.', ',').' ₫' : '—' ?>
                        </td>
                        <td class="text-end table-warning fw-semibold">
                            <?= $veh['has_rule'] ? number_format($veh['amount_base'], 0, '.', ',').' ₫' : '—' ?>
                        </td>
                        <td class="text-end">
                            <?php if ($veh['toll_included']): ?>
                            <span class="text-muted small">Đã bao gồm</span>
                            <?php elseif ($veh['amount_toll'] > 0): ?>
                            <span class="text-warning fw-semibold"><?= number_format($veh['amount_toll'], 0, '.', ',') ?> ₫</span>
                            <?php else: ?>—<?php endif; ?>
                        </td>
                        <td class="text-end">
                            <?= $veh['amount_surcharge'] > 0
                                ? '<span class="text-info fw-semibold">'.number_format($veh['amount_surcharge'], 0, '.', ',').' ₫</span>'
                                  .' <small class="text-muted">('.$veh['sunday_trips'].' CN)</small>'
                                : '—' ?>
                        </td>
                        <td class="text-end fw-bold table-success fs-6">
                            <?= $veh['has_rule'] ? number_format($veh['amount_total'], 0, '.', ',').' ₫' : '<span class="text-muted">—</span>' ?>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                    </tbody>
                    ...
                    <table class="table table-sm table-hover align-middle mb-0" style="font-size:0.78rem">
                        ...
                        foreach ($stmt['vehicles'] as $plate => $veh):
                            foreach ($veh['trips'] as $t):
                                ...
                                $statusLbl = ['completed'=>'✅ Hoàn thành','confirmed'=>'👍 Đã duyệt','rejected'=>'❌ Từ chối'][$t['status']] ?? $t['status'];
                        ...
                        <tr class="<?= $t['status']==='confirmed' ? 'table-success bg-opacity-10' : ($t['status']==='rejected' ? 'table-danger bg-opacity-10' : '') ?>">
                            ...
                            <td class="text-nowrap">
                                ...
                                <?php if ($t['is_sunday']): ?><span class="badge bg-warning" style="font-size:0.6rem">CN</span><?php endif; ?>
                            </td>
                            ...
                            <td class="text-end fw-bold text-primary"><?= $t['total_km'] ? number_format($t['total_km'], 0).' km' : '—' ?></td>
                            <td class="text-end"><?= $t['toll_fee'] ? number_format($t['toll_fee'], 0, '.', ',').' ₫' : '—' ?></td>
