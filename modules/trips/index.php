...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete_trip') {
    $delId = (int)$_POST['trip_id'];
    // Chỉ cho xóa khi status là draft, submitted, rejected
    $chk = $pdo->prepare("SELECT status FROM trips WHERE id = ?");
    $chk->execute([$delId]);
    $chkRow = $chk->fetch();
    if ($chkRow && in_array($chkRow['status'], ['draft', 'submitted', 'rejected'])) {
        $pdo->prepare("DELETE FROM trips WHERE id = ?")->execute([$delId]);
        $_SESSION['flash'] = ['type' => 'success', 'msg' => '✅ Đã xóa chuyến xe.'];
    } else {
        $_SESSION['flash'] = ['type' => 'danger', 'msg' => '❌ Không thể xóa chuyến đã được duyệt.'];
    }
    header('Location: index.php?' . http_build_query(array_diff_key($_GET, ['action' => ''])));
    exit;
}
...
// Các trạng thái được phép sửa/xóa
                ...
                <table class="table table-bordered table-hover align-middle mb-0"
                    ...
                    <thead style="background:#f8f9fa">
                        <tr class="text-center">
                            <th class="align-middle" style="width:35px">STT</th>
                            <th class="align-middle">Người lái</th>
                            <th class="align-middle">Ngày</th>
                            <th class="align-middle text-danger">BIỂN SỐ XE</th>
                            <th class="align-middle">TẢI TRỌNG</th>
                            <th class="align-middle text-danger">Khách Hàng</th>
                            <th class="align-middle text-danger">Điểm Đi</th>
                            <th class="align-middle text-danger">Điểm Đến</th>
                            <th class="align-middle text-danger">KM Đi</th>
                            <th class="align-middle text-danger">KM Kết thúc</th>
                            <th class="align-middle text-danger">Tổng KM</th>
                            <th class="align-middle">Vé Cầu Đường</th>
                            <th class="align-middle">Ghi chú</th>
                            <th class="align-middle">Trạng Thái</th>
                            <th class="align-middle">KH Duyệt</th>
                    ...
                    <?php foreach ($trips as $i => $t):
                        ...
                        $canEdit = in_array($t['status'], $editableStatuses)
                                   && (can('trips', 'create') || can('trips', 'view_all'));
                    ?>
                    <tr class="<?= $t['status'] === 'rejected' ? 'table-danger bg-opacity-10' : '' ?>">
                        <td class="text-center text-muted small"><?= $i + 1 ?></td>
                        <td>
                            <div class="fw-semibold"><?= htmlspecialchars($t['driver_name']) ?></div>
                            <div class="text-muted" style="font-size:0.72rem"><?= $t['trip_code'] ?></div>
                        </td>
                        ...
                        <td class="text-center">
                            <div class="d-flex gap-1 justify-content-center flex-wrap">
                                ...
                                <a href="detail.php?id=<?= $t['id'] ?>"
                                   class="btn btn-xs btn-outline-info" title="Chi tiết">
                                ...
                                <a href="edit.php?id=<?= $t['id'] ?>"
                                   class="btn btn-xs btn-outline-warning" title="Chỉnh sửa">
                                    <i class="fas fa-edit"></i>
                                </a>

                                <!-- Xóa -->
                                <form method="POST" class="d-inline"
                                      onsubmit="return confirm('Xác nhận xóa chuyến <?= htmlspecialchars(addslashes($t['trip_code'])) ?>?')">
                                ...
                                <a href="confirm.php?id=<?= $t['id'] ?>&action=approve"
                                   class="btn btn-xs btn-outline-success" title="Duyệt"
                                   onclick="return confirm('Duyệt chuyến này?')">
                                    <i class="fas fa-check"></i>
                                </a>
                                <!-- Từ chối -->
                                <a href="confirm.php?id=<?= $t['id'] ?>&action=reject"
                                   class="btn btn-xs btn-outline-danger" title="Từ chối">
