...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ── MỚI: Tạo user mới và thêm vào customer luôn ─────────
...
// Load price rules của b��ng giá được chọn
...
// Logic: 1 user = 1 công ty
// Hiện users role=customer chưa thuộc công ty nào (hoặc đã inactive ở công ty khác)
$availableUsers = $pdo->prepare("
    ...
    WHERE r.name = 'customer'
      ...
      AND u.id NOT IN (
          -- Loại users đang ACTIVE ở công ty KHÁC
          SELECT cu2.user_id
          FROM customer_users cu2
          WHERE cu2.is_active = TRUE
            AND cu2.customer_id != ?
      )
      AND u.id NOT IN (
          -- Loại users đang ACTIVE trong chính KH này
          SELECT cu3.user_id
          FROM customer_users cu3
          WHERE cu3.is_active = TRUE
            AND cu3.customer_id = ?
      )
...
// Random password cho form tạo user mới
